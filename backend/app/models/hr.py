...
class DepartmentBase(SQLModel):
    ...
    external_id: Optional[int] = Field(
        default=None, index=True)  # id in hr_department
...
class PositionBase(SQLModel):
    name: str = Field(index=True, max_length=255)
    external_id: Optional[int] = Field(
        default=None, index=True)  # position_id in ZKTime
...
class EmployeeBase(SQLModel):
    ...
    external_id: Optional[int] = Field(
        default=None, index=True)  # id in hr_employee
...
class Employee(EmployeeBase, table=True):
    ...
    attendance_logs: List["AttendanceLog"] = Relationship(
...
class EmployeesPublic(BaseModel):
